...
// gcc add.c -o app $(pkg-config --cflags --libs gtk+-3.0)  && ./app
...
void do_calculate(GtkWidget *calculate, gpointer data){
    int num1 = atoi(gtk_entry_get_text(GTK_ENTRY(number1)));
    int num2 = atoi(gtk_entry_get_text(GTK_ENTRY(number2)));
    char buffer[32];
...
int main(int argc, char *argv[]){
    ...
    gtk_window_set_title(GTK_WINDOW(window), "GTK Test Calculater");
